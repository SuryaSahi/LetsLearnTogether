class Solution1 {
    public int maxScoreWords(String[] words, char[] letters, int[] score) {
        ...
        int ans = 0;
        backtrack(words ,  score , count , letterCount , ans , 0 , 0);
        return ans;
    }
    
    private int backtrack(String[] words , int[] score , int[] count , int[] letterCount , int ans , int index , int temp){
        for(int i = 0; i<26; i++){
        ...
        for(int i = index; i<words.length; i++){
            ...
            backtrack(words , score , count , letterCount , ans , index + 1 , temp);
